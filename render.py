...
from schema import Post, PostAdapter, SentencePost, VocabularyPost
...
def load_post(number: int) -> Post:
    ...
    try:
        return PostAdapter.validate_python(raw)
...
def sentence_count_through(number: int) -> int:
    count = 0
    for n in existing_numbers():
        if n > number:
            break
        raw = yaml.safe_load(post_path(n).read_text(encoding="utf-8")) or {}
        if raw.get("type", "sentence") == "sentence":
            count += 1
    return count
...
def render_html_files(post: Post) -> list[tuple[str, Path]]:
    ...
    if isinstance(post, VocabularyPost):
        slides = [
            ("01-overview", "vocabulary/overview.html", {"post": post, "css": css}),
            ("02-chinese", "vocabulary/language.html", {"post": post, "css": css, "lang_key": "chinese"}),
            ("03-japanese", "vocabulary/language.html", {"post": post, "css": css, "lang_key": "japanese"}),
            ("04-german", "vocabulary/language.html", {"post": post, "css": css, "lang_key": "german"}),
            ("05-use-them", "vocabulary/use-them.html", {"post": post, "css": css}),
            ("06-reference", "vocabulary/reference.html", {"post": post, "css": css}),
        ]
    else:
        common = {"sentence_count": sentence_count_through(post.number)}
        slides = [("01-cover", "cover.html", {"post": post, "css": css, **common})]
        for index, (attr, code) in enumerate(LANGS, start=2):
            slides.append((f"{index:02d}-{attr}", "language.html", {"post": post, "language": getattr(post, attr), "lang_key": attr, "lang_code": code, "css": css, **common}))
        slide_num = 5
        if post.connection and post.connection.has_content():
            slides.append((f"{slide_num:02d}-connections", "connections.html", {"post": post, "css": css, **common})); slide_num += 1
        if post.field_note and post.field_note.strip():
            slides.append((f"{slide_num:02d}-field-note", "field-note.html", {"post": post, "css": css, **common}))

    rendered=[]
    for stem, template, context in slides:
        body=jinja.get_template(template).render(**context)
        path=out/f"{stem}.html"; path.write_text(body,encoding="utf-8"); rendered.append((stem,path))
    preview=jinja.get_template("preview.html").render(post=post,slides=[(s,p.name) for s,p in rendered])
